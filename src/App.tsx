      ...
      <main className="flex-1 flex flex-col overflow-hidden bg-[#F9FAFB] relative">
        ...
        <header className="h-[52px] bg-white border-b border-[#E5E7EB] px-6 flex items-center justify-between shrink-0 relative z-10">
          <div className="text-[13px] text-[#6B7280] flex items-center gap-1.5">
            ...
            <strong className="text-[#111827] font-semibold">
              {currentPage === 'ai-agent' ? 'AI 에이전트 분석 결과' : currentPage === 'risk-analysis' ? '리스크 분석' : currentPage === 'ai-chat' ? 'AI 대화' : '대시보드'}
            </strong>
          ...
          <div className="flex items-center gap-3">
            {currentPage === 'ai-chat' ? (
              <>
                <span className="text-[11.5px] text-[#059669] font-medium flex items-center gap-1">
                  <div className="w-1.5 h-1.5 rounded-full bg-[#10B981]" /> 실시간
                </span>
                <button className="bg-[#1D4ED8] text-white text-xs font-semibold px-3.5 py-1.5 rounded-md flex items-center gap-1.5">
                  <Bot size={14} /> AI에게 질문하기
                </button>
              </>
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar relative">
          <AnimatePresence mode="wait">
            ...
            {currentPage === 'ai-chat' && <AIChatPage onClose={() => setCurrentPage('dashboard')} />}
          </AnimatePresence>
        </div>
      </main>
    </div>
